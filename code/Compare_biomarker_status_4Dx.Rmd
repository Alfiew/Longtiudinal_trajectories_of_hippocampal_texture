...
# Filter data
Take only those with Tau status - PET tau? No, not very many of those.
```{r}
...
pc_dataset$PTAU_ABETA_status <- factor(pc_dataset$PTAU_ABETA_status, levels = c(0, 1))
...
bl_dataset$PTAU_ABETA_status <- as.factor(bl_dataset$PTAU_ABETA_status)
bl_dataset$ABETA_status <- as.factor(bl_dataset$ABETA_status)
bl_dataset$PTAU_status <- as.factor(bl_dataset$PTAU_status)
```
...
# 
# bl_dataset %>%
#   group_by(DX_TxBl_Ab, PTAU_status) %>%
#   summarise(n=n())
# 
# bl_dataset %>%
#   group_by(DX_TxBl_Ab, PTAU_ABETA_status) %>%
#   summarise(n=n())

# Save ID list
Tx_group_compare_list <- bl_dataset %>%
  # filter(!is.na(DX_TxBl_Ab),!is.na(ABETA_status)) %>% # Redundant
  select(PTID, VISCODE) %>%
  mutate(Tx_group_compare = "Y")

save(Tx_group_compare_list, file="../data/ID_lists/Tx_group_compare_list.Rdata")
```

# Do stats and plot data

Answer two questions separately. 
1) Can Tx identify Amyloid positivity? (In CN people)
2) Can Tx identify Tau positivity? (In CN people) 

```{r}
# var <- pc_vars[4]
# out_list <- anovas_hipp
# loop_count <- 1
...
compare_bmks <- function(var, out_list = NULL, loop_count = NULL) {
  ...
  print(anova(dem.aov))
    ...
    if (is.null(out_list)){
      ...
      names(output)[1:4] <- c(paste(var,'Dx4', sep='_'),paste(var,'cn', sep='_'),paste(var,'mci', sep='_'),paste(var,'dem', sep='_'))

    } else {
      output <- out_list
      output[[(4*loop_count)-3]] <- anova(Dx4.aov)
      output[[(4*loop_count)-2]] <- anova(cn.aov)
      output[[(4*loop_count)-1]] <- anova(mci.aov)
      output[[4*loop_count]] <- anova(dem.aov)
      names(output)[((4*loop_count)-3):(4*loop_count)] <- c(paste(var,'Dx4', sep='_'),paste(var,'cn', sep='_'),paste(var,'mci', sep='_'),paste(var,'dem', sep='_'))
...
```{r, eval=FALSE}
save(bl_dataset, summary_h, summary_h_cn, summary_h_mci, summary_h_dem, anovas_hipp, file = "../Publication figures/data/compare_biomarker_status_4Dx.Rdata")
```
